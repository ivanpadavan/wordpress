...
class TableScreen {
	...
	public function __construct() {
		...
		add_action( 'ac/table/list_screen', array( $this, 'add_events_filter_vars' ) );
		add_action( 'parse_query', array( $this, 'parse_query' ), 51 );
	}

	public function table_scripts() {
		wp_enqueue_style( 'aca-ec-table', ac_addon_events_calendar()->get_url() . 'assets/css/table.css', array(), ac_addon_events_calendar()->get_version() );
	}
	...
	public function add_events_filter_vars( $list_screen ) {
		...
		add_action( 'pre_get_posts', array( $this, 'events_query_callback' ) );
	}
	...
	private function filter_on_venue( $value ) {
		...
		$this->filter_vars['meta_query'][] = array(
