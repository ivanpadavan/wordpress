<?php
/**
 * Ezy Featured Post Widget.
 *
 * @since 1.0.0
 */
if (!class_exists('Ezy_Featured_Post')) :
  /**
     * Highlight Post widget class.
     *
     * @since 1.0.0
     */
    class Ezy_Featured_Post extends WP_Widget
    {
       function __construct()
        {
            $opts = array(
                'classname'   => 'ezy-featured-post',
                'description' => esc_html__('Displays Featured Post in Home Page. Place it in Home Widget Area Top.', 'ezy'),
            );
            parent::__construct('ezy-featured-post', esc_html__('CT Featured Posts', 'ezy'), $opts);
        }
        ...
        function form($instance)
        {
            ...
            <p class="custom-dropdown-posts">
               <label for="<?php echo esc_attr($this->get_field_name('cat')); ?>">
                <strong><?php esc_html_e('Select Category: ', 'ezy'); ?></strong>
                </label>
                <?php
                $cat_args = array(
                    'orderby'         => 'name',
                    ...
                    'show_option_all' => esc_html__('Show Recent Posts', 'ezy')
                );
                wp_dropdown_categories($cat_args);
                ?>
            </p>
            <p>
                <label for="<?php echo esc_attr($this->get_field_id('post-number')); ?>"><?php esc_html_e('Number of Posts to Display:', 'ezy'); ?></label>
...
add_action( 'widgets_init', 'ezy_featured_post' ); 
function ezy_featured_post(){     
    register_widget( 'Ezy_Featured_Post' );
}